...
from codesherpa.contracts.types import PackedContext, RetrievalSource
from codesherpa.embed.engine import EmbeddingEngine
from codesherpa.gitlayer.repo import open_repo, repo_root
...
class IndexNotBuiltError(RuntimeError):
    """The repo has no .sherpa/index.db yet — `sherpa init` first."""
...
def build_retriever(repo_path: str, config: RetrievalConfig | None = None):
    ...
    Consumed by ``python -m codesherpa.mcp_server`` / ``sherpa serve``.
    Deliberately does NOT sync, embed, or load any model: index building and
    embedding belong to ``sherpa init``/``sync`` (Phase 5 §3e — the MCP
    ...
    if not db.is_file():
        raise IndexNotBuiltError(
            f"no index at {db} — run `sherpa init` in the repository first"
        )
