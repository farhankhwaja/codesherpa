...
``.repographignore`` supports a pragmatic gitignore subset (documented in
``RepographIgnore``): comments, blank lines, ``dir/`` prefixes, ``/``-anchored
...
IGNORE_FILENAME = ".repographignore"
...
class RepographIgnore:
    """Patterns from ``.repographignore`` (empty rule set if absent)."""
    ...
    @classmethod
    def load(cls, repo_root: Path) -> "RepographIgnore":
...
class IgnoreRules:
    ...
    def __init__(self, user_rules: RepographIgnore) -> None:
        self.user_rules = user_rules

    @classmethod
    def load(cls, repo_root: Path) -> "IgnoreRules":
        return cls(RepographIgnore.load(repo_root))
