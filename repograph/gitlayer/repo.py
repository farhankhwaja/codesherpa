...
# The hook must NEVER break a git operation: no repograph on PATH -> no-op,
# sync failure -> swallowed (sync itself logs).
_HOOK_MARKER = "# installed by repograph"
_HOOK_BODY = f"""#!/bin/sh
{_HOOK_MARKER} — keeps .repograph/index.db fresh after history changes
command -v repograph >/dev/null 2>&1 && repograph sync --quiet || true
"""
...
def install_hooks(repo: pygit2.Repository) -> list[str]:
    ...
    for name in HOOK_NAMES:
        ...
        if not hook.exists():
            ...
            if _HOOK_MARKER in existing or "repograph sync" in existing:
                continue
            addition = (
