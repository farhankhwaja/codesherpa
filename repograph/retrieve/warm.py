"""Index-wide embedding pass — owned by ``repograph init`` / ``repograph sync``.
...
missing (repograph/mcp_server/server.py).
...
from repograph.embed.engine import EmbeddingEngine, model_is_cached
...
def embed_index(
    ...
    *,
    ...
    triggers a model download (first downloads belong to ``repograph init``).
    """
    from repograph.retrieve.config import RetrievalConfig  # local: avoid cycles
