...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_init.add_argument(
        "--no-embed",
        action="store_true",
        help="Skip the embedding pass (semantic search stays cold until `repograph sync`).",
    )
...
def _embed_progress_printer(stream=None):
    ...
    def progress(done: int, total: int) -> None:
        ...
        if tty:
            ...
            print(f"\rrepograph: embedding chunks {done}/{total} ({pct}%)", end=end, file=out, flush=True)
        else:
            step = pct // 10
            if step > last_step or done >= total:
                last_step = step
                print(f"repograph: embedding chunks {done}/{total} ({pct}%)", file=out, flush=True)
...
def _embed_pass(root, *, quiet: bool, hook_safe: bool = False) -> int:
    ...
    downloads belong to a foreground ``repograph init``/``sync``.
    """
    from repograph.gitlayer.sync import default_db_path
    from repograph.retrieve import RetrievalConfig
    ...
    try:
        ...
        if missing and not quiet:
            if not model_is_cached(config.model_cache_dir, config.embed_model):
                print(
                    f"repograph: downloading embedding model {config.embed_model} "
                    f"(one-time, ~0.5 GB) to {config.model_cache_dir} …",
                    file=sys.stderr,
                    flush=True,
                )
            print(
                f"repograph: embedding {missing} chunks (CPU; incremental — "
...
def _cmd_init(args: argparse.Namespace) -> int:
    ...
    if args.no_embed:
        print(
            "repograph: --no-embed: semantic search stays cold until `repograph sync`.",
            file=sys.stderr,
        )
        return 0
...
def _cmd_search(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        print(f"repograph search: {exc}", file=sys.stderr)
        return 1
    ...
    try:
        ...
        if not packed.results:
            print("repograph search: no results.", file=sys.stderr)
            return 0
