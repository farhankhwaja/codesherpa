...
The `graph/index.py` TODO(upgrade) (per-blob extraction facts) is DONE — D48.
...
- feat/bench-and-graph-cache: per-blob graph extraction cache (D48, new
  `graph_facts` table + `graph_facts_tag` invalidation), real `sherpa bench`
  (D49, logic moved to `codesherpa/bench.py`; `tests/bench_indexing.py` is now
  a thin wrapper), and a quadratic Go import-resolution fix found while
  measuring (D50). grafana `pkg/` no-op sync 164.93 s → 7.52 s; suite 351→363
...
- Graph cache (D48): if you change what `extract._extract_file` collects, BUMP
